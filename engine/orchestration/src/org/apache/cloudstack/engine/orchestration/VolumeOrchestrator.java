...
public class VolumeOrchestrator extends ManagerBase implements VolumeOrchestrationService, Configurable {
    ...
    public DiskProfile allocateRawVolume(Type type, String name, DiskOffering offering, Long size, Long minIops, Long maxIops, VirtualMachine vm, VirtualMachineTemplate template, Account owner) {
        ...
        if (vm.getType() == VirtualMachine.Type.User) {
            UsageEventUtils.publishUsageEvent(EventTypes.EVENT_VOLUME_CREATE, vol.getAccountId(), vol.getDataCenterId(), vol.getId(), vol.getName(), offering.getId(), offering.getUuid(), null, null, size,
    ...
    public DiskProfile allocateTemplatedVolume(Type type, String name, DiskOffering offering, Long rootDisksize, Long minIops, Long maxIops, VirtualMachineTemplate template, VirtualMachine vm, Account owner) {
        ...
        if (vm.getType() == VirtualMachine.Type.User) {
            ...
            UsageEventUtils.publishUsageEvent(EventTypes.EVENT_VOLUME_CREATE, vol.getAccountId(), vol.getDataCenterId(), vol.getId(), vol.getName(), offeringId, offering.getUuid(), vol.getTemplateId(), template.getUuid(), size,
                    Volume.class.getName(), vol.getUuid(), vol.isDisplayVolume());
    ...
    public void confirmMigration(VirtualMachineProfile vm, final long srcHostId, final long destHostId, boolean migrationSuccessful) {
        List<VolumeVO> vols = _volsDao.findUsableVolumesForInstance(vm.getId());
        StoragePool poolToCleanup = storageManager.findLocalStorageOnHost((migrationSuccessful ? srcHostId : destHostId));
        StoragePool pool = storageManager.findLocalStorageOnHost((migrationSuccessful ? destHostId : srcHostId));
        Volume volume = null;
        for (VolumeVO vol : vols) {
            if (vm.getType() == VirtualMachine.Type.User && vol.getVolumeType().equals(Type.ROOT)) {
                volume = vol;
